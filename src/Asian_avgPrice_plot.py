...
con precio promedio, variando el parámetro de volatilidad.
...
for sigma in sigmaV:
    ...
    # Montecarlo con media aritmética
    ...
    # Montecarlo con media geométrica
...
plt.xlabel(r"$\sigma$")
plt.ylabel(r"$C(0, 0)$")
plt.title("Precio de opciones Asiáticas (precio promedio) en función de la volatilidad", pad=10)
plt.legend(loc="upper left", frameon=False)
plt.grid(True, linestyle="--", linewidth=0.5, alpha=0.7)
plt.tight_layout()
output_path = "resultados"
os.makedirs(output_path, exist_ok=True) # Crear directorio si no existe
plt.savefig(os.path.join(output_path, "compGRAPH_allPrice.pdf"))
plt.show()